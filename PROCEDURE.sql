...
CREATE OR ALTER PROC SP_THEM_SACH 
    @ma_sach CHAR(10), 
    @ma_the_loai CHAR(10),
    @ma_nxb CHAR(10), 
...
CREATE OR ALTER PROC SP_SUA_SACH 
    @ma_sach CHAR(10), 
    @ten_sach NVARCHAR(200), 
    @nam_xuat_ban INT, 
    @so_luong_cuon INT
...
CREATE OR ALTER PROC SP_XOA_SACH 
    @ma_sach CHAR(10)
AS
    IF NOT EXISTS (SELECT * FROM SACH WHERE ma_sach = @ma_sach)
...
CREATE OR ALTER PROC SP_THEM_CUONSACH
    @ma_cuon_sach CHAR(10),
    @ma_sach CHAR(10),
    @ma_vi_tri CHAR(10),
...
CREATE OR ALTER PROC SP_SUA_CUONSACH
    @ma_cuon_sach CHAR(10),
    @ma_vi_tri CHAR(10),
    @trang_thai NVARCHAR(50)
AS
...
CREATE OR ALTER PROC SP_XOA_CUONSACH
    @ma_cuon_sach CHAR(10)
AS
    IF NOT EXISTS (SELECT * FROM CUONSACH WHERE ma_cuon_sach = @ma_cuon_sach)
...
CREATE OR ALTER PROC SP_LICHSUMUON_DOCGIA
    @ma_doc_gia CHAR(10)
AS
BEGIN
    SELECT 
        DOCGIA.ma_doc_gia,
        DOCGIA.ho_ten AS ten_doc_gia,
        PHIEUMUON.ma_phieu_muon,
        PHIEUMUON.ngay_muon,
        PHIEUMUON.han_tra,
        SACH.ten_sach,
        CHITIETPHIEUMUON.tinh_trang AS tinh_trang_khi_muon
...
CREATE OR ALTER PROC SP_LICHSUTRA_DOCGIA
    @ma_doc_gia CHAR(10)
AS
BEGIN
    SELECT
        DOCGIA.ma_doc_gia,
        DOCGIA.ho_ten AS ten_doc_gia,
        PHIEUTRA.ma_phieu_tra,
        PHIEUTRA.ngay_tra,
        SACH.ten_sach,
        CHITIETPHIEUTRA.tinh_trang_khi_tra AS tinh_trang
...
CREATE OR ALTER PROC SP_XEMPHIEUMUON
    @ma_phieu_muon CHAR(10)
AS
BEGIN
    SELECT 
        CUONSACH.ma_cuon_sach,
        SACH.ten_sach,
        CHITIETPHIEUMUON.tinh_trang AS tinh_trang_khi_muon
...
CREATE OR ALTER PROC SP_XEMPHIEUTRA
    @ma_phieu_tra CHAR(10)
AS
BEGIN
    SELECT 
        CUONSACH.ma_cuon_sach,
        SACH.ten_sach,
        CHITIETPHIEUTRA.tinh_trang_khi_tra AS tinh_trang_khi_tra
